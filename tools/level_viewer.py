    ...
    def blit_fresh_tile_to_img_unzoomed_dualpalette(
        ...
        for y in range(8):
            ...
            if outrow and len(outrow[0][1]) == 16:
                bigaccum.append("{" + " ".join(outrow[0][1]) + "}")
                if y == 8 - 1:
                    eval_pile.append(
                        f"{img_name} put {{{' '.join(bigaccum)}}} -to {px} {py + (y + 1) - len(bigaccum)}"
                    )
            else:
                if bigaccum:
                    eval_pile.append(
                        f"{img_name} put {{{' '.join(bigaccum)}}} -to {px} {py + y - len(bigaccum)}"
                    )
                    bigaccum.clear()
                for outrow_x, outdata0, outdata1 in outrow:
                    ...
                    if outdata0:
                        eval_pile.append(
                            f"{img_name} put {{{{{' '.join(outdata0)}}}}} -to {px + outrow_x} {py + y}"
                        )
                        if outdata1:
                            eval_pile.append(
                                f"{img_name} put {{{{{' '.join(outdata1)}}}}} -to {px + outrow_x + 8} {py + y}"
                            )
